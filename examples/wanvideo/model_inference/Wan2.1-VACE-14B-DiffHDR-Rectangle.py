...
output_dir = "/scratch/eli/disney/DiffSynth-Studio/output/02/"
...
# Build a static rectangular mask (white = inpaint, black = keep).
mask_img = Image.fromarray(np.zeros((H, W), dtype=np.uint8))
draw = ImageDraw.Draw(mask_img)
draw.rectangle([int(0.5 * W), int(0.2 * H), int(0.7 * W), int(0.6 * H)], fill=255)
mask_rgb = mask_img.convert("RGB")
mask_frames = [mask_rgb] * len(frames)
...
mask_alpha = np.array(mask_img, dtype=np.float32) / 255.0  # H x W, 0..1
overlay_frames = []
for frame in frames:
...
video = pipe(
    prompt="曝光过度：恢复灯泡和灯光，恢复它们最亮的部分和高光；曝光不足：恢复天空和星空，恢复狮子的细节; HDR, 非常详细, 电影灯光, 高对比度", # Overexposed: Recover the light bulbs and lights, recover their brightest points and highlights; Underexposed: Recover the sky and the stars in the sky, recover the details on the lion; HDR, extremely detailed, cinematic lighting, high contrast.
